<script lang="ts">
	...
	const initialMessagesCount = messagesCount;
	...
	const getScrollAnchor = (container: HTMLElement): ScrollAnchor | null => {
		const containerRect = container.getBoundingClientRect();
		const anchorEl = Array.from(
			container.querySelectorAll<HTMLElement>('[role="listitem"][id^="message-"]')
		).find((el) => {
	...
	const restoreScrollAnchor = (
		...
		if (anchor) {
			const anchorEl = Array.from(
				container.querySelectorAll<HTMLElement>('[role="listitem"][id^="message-"]')
			).find((el) => el.id === anchor.id);
			if (anchorEl) {
				const containerRect = container.getBoundingClientRect();
				const offset = anchorEl.getBoundingClientRect().top - containerRect.top;
				container.scrollTop += offset - anchor.offset;
				return;
			}
		}

		container.scrollTop = container.scrollHeight - fallbackScrollHeight + fallbackScrollTop;
	};
	...
	const loadMoreMessages = async () => {
		...
		await tick();
		restoreScrollAnchor(element, anchor, previousScrollHeight, previousScrollTop);
