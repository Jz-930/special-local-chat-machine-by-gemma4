...
start "Open-WebUI Backend" cmd /k "cd /d ""%BACKEND_DIR%"" && call venv\Scripts\activate.bat && IF NOT EXIST "".env_ready"" ( echo [Backend] Installing lightweight requirements... && pip install -r requirements-novel.txt && echo > .env_ready ) && set OLLAMA_BASE_URL=https://sb8kkgscyvffwh-11434.proxy.runpod.net && set WEBUI_AUTH=False && set ENABLE_TITLE_GENERATION=False && set ENABLE_TAGS_GENERATION=False && set WEBUI_SECRET_KEY=dev_local_secret_123 && set FORWARDED_ALLOW_IPS=127.0.0.1 && echo [Backend] Starting server... && call start_windows.bat"
